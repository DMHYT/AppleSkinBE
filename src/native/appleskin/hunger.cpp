...
void AppleHungerModule::drawExhaustionOverlay(ScreenContext& ctx, float exhaustion, float x, float y) {
...
void AppleHungerModule::drawSaturationOverlay(ScreenContext& ctx, float saturation, float saturationGained, float right, float top, float alpha) {
...
void AppleHungerModule::drawHungerOverlay(ScreenContext& ctx, float hungerRestored, float foodLevel, float right, float top, float alpha, bool useRottenTextures) {
...
void AppleHungerModule::generateHungerBarOffsets(float right, float top, int ticks, LocalPlayer* player) {
    ...
    for(int i = 0; i < 10; ++i) {
        float x = right - i * 8 - 9;
        int y = top;
        if(shouldAnimateFood) y += rand->nextInt(3) - 1;
...
void AppleHungerModule::onPreRender(ScreenContext* ctx, Vec2* position) {
    if(AppleMainModule::ModConfig::SHOW_FOOD_EXHAUSTION_UNDERLAY) {
        drawExhaustionOverlay(*ctx, GlobalContext::getLocalPlayer()->getExhaustion(), position->x, position->y);
    }
}
...
void AppleHungerModule::onRender(ScreenContext* ctx, Vec2* position, int ticks) {
    if(AppleMainModule::shouldRenderAnyOverlays()) {
        ...
        if(AppleMainModule::ModConfig::SHOW_SATURATION_OVERLAY) {
            drawSaturationOverlay(*ctx, playerSaturation, 0.0f, position->x + 1, position->y, 1.0f);
        }
...
void AppleHungerModule::initialize() {
    ...
    HookManager::addCallback(
        ...
        LAMBDA((HudHungerRenderer* renderer, MinecraftUIRenderContext& renderContext, ClientInstance& clientInstance, UIControl& control, int someInt, RectangleArea& area), {
            onPreRender(renderContext.getScreenContext(), control.getPosition());
